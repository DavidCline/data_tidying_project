...
In this project we will review retail sales data and department store sales data in the U.S.

First step is to load the packages we will need for the project:
...
Next we will read in the data from the data.world website.
...
Then we save the raw data for future reference, guarding against website changes or updates that might eliminate the data we are using for this analysis, this in the interests of reproducibility.

### Save Raw Data

```{r save-data, warning=FALSE} 
## use saveRDS() to save each object as a .rds file 
saveRDS(df1, "//cloud//project//data_tidying_project//data//raw_data//df_department.rds")
saveRDS(df2, "//cloud//project//data_tidying_project//data//raw_data//df_retail.rds")
```

Now we have to wrangle our way to several changes to focus on only the information we want and to combine data from two tables into one with more readable variable names.
...
Pictures being worth thousands of words, we now visualize the data we have summarized.
